...
class Handler(webapp2.RequestHandler):
    ...
    def get_user_by_cookie(self):
        ...
        if cookie == "" or cookie is None or cookie == " ":
            ...
            user_id, hash_val, salt = self.request.cookies.get("user_id").split('|')
            return self.val_user_and_hash(user_id, hash_val+'|'+salt)
            


class SignUpPage(Handler):
    """Handler class renders htmls for SignUp page. This handles signup http get and post requests"""
    def get(self):
    ...
    def post(self):
        """Validates username, password and email as per the rules. 
            1. Username should have 3 or more character and there should be no whitespace inbetween.
        ...
        elif db.GqlQuery("select * from User where name = :1" , username).count():
        ...
        if error_name or error_password or error_verify or error_email:
            self.render('signup.html', username=username, email=email, error_name = error_name,
            ...
            self.response.headers.add_header('Set-Cookie', 'user_id=%s' % cookie_name)
            self.redirect('/blog')


class LoginPage(Handler):
    """Handler class renders htmls for login page. This handles login http get and post requests"""
    def get(self):
        ...
        if not (username and password):
            ...
            if not user_info.count():
                ...
                if self.val_hash_pass(username, password, user_hash):
                    cookie_name = '%s|%s' %(user_info.get().key().id(),str(user_hash))
...
class BlogPost(Handler):
    ...
    def post(self):
        ...
        if subject and content:
            ...
            self.render('blogpost.html', error=error, subject=subject, content=content, post_title=post_title, username = self.request.user.name)


class UserBlog(Handler):
    """Renders user blog, adds functionality to edit user blog and delete user blog"""
    @authenticated
    def get(self, blog_id):
        """Renders user blog and let them edit their blogs"""
        blog = Blog.get_by_id(int(blog_id))
        logging.debug("user = %s, blog_user = %s" %(self.request.user.key().id(), blog.name.key().id()))
        if not blog:
            self.response.set_status(400)
            self.redirect('/blog/newpost')
        else:
            edit = self.request.get('edit', False)
            comment_count = UserComment.all().filter('blog =', blog).count() or 0
            blog_comments = UserComment.all().filter('blog =', blog).order('-created_time')
            logging.info("comment_count = %s, blog=%s, edit=%s", comment_count, blog.name, type(str(edit)))
            ...
            if self.request.get('edit') == 'true' and self.request.user.key().id() == blog.name.key().id():
                logging.debug("UserBlog::get edit: {}".format(edit))
                self.render('blogpost.html',post_title="Edit User Post", blog=blog, user = self.request.user) 
            else:
                self.render('userpost.html', blog=blog, count=comment_count, likes=likes_count, user_edit_delete_state=user_edit_delete_state, blog_comments=blog_comments, user=self.request.user)

    @authenticated
    def post(self, blog_id):
        """Commits User blog after edit"""
        logging.debug("Blog.get_by_id(int(blog_id)): %s", dir(Blog.get_by_id(int(blog_id))))
        blog = Blog.get_by_id(int(blog_id))
        if not blog:
        ...
        if not blog:
            ...
            if not json_body:
                ...
                if state == "comment":
                    ...
                    user_comment = UserComment(blog=blog, user=self.request.user, comment=comment)
                    user_comment.put()
                    count = UserComment.all().filter('blog =', blog).count() or 0
                    count += 1
                    status = 200
                    response = {
                        'comment_count': count,
                        ...
                        'created': user_comment.created_time.strftime("%b %d, %Y")
                    }
                elif state == "edit-comment":
                    newComment = json_body.get("comment_content", None)
                    comment_id = json_body.get("comment_id", None)
                    logging.info("state: %s , comment_id: %s" %(state, comment_id))
                    user_comment = UserComment.get_by_id(int(comment_id))
                    logging.info("user_comment = %s", dir(user_comment))
                    if user_comment and self.request.user.key().id() == user_comment.user.key().id() and user_comment.blog.key().id() == blog.key().id():
                        ...
                        status = 400
                        response = {
                            'msg': 'either comment doesnot exists or user logged in/blog id and comment user/blog id are not same.'
                        }
                else:
                    state = 500
                    response = {
...
class AllBlogs(Handler):
    """Displays all blogs with like counts for the user. 
       ...
       by the user and which are not already liked. Also mark/unmark blog as favorite"""
    @authenticated
    def get(self):
        """Displays all blogs with like counts for the user. 
           Also it tell if the blog is marked favorite by the user"""
        ...
        if all.count():
            ...
            for a in all:
                blog_id_comment_count[a.key().id()] = UserComment.all().filter('blog =', a).count()
            #comment_count = UserCommentCount.all()
            self.render('allblogs.html', data=all, fav_blog_id_list=fav_blog_id_list, blog_id_comment_count=blog_id_comment_count) #, comment_count=comment_count)
        else:
            ...
            if blog_id:
                ...
                if not blog:
                    ...
                    if fav_state is not None:
                        # toggle fav state - if fav_status is true then change to false
                        fav_state = UserFavs.update_fav(self.request.user, blog, fav_state)
                        ...
                        if fav_state is not None:
                            ...
                            status = 400
                            response = {
                                "msg": "fav update unsuccessfull. something went wrong"
                            }
                    # In Like count increase block
                        ...
                        if self.request.user.key().id() == blog.name.key().id():
                            ...
                            if not UserLikes.user_has_liked(self.request.user, blog):
                        ...
                        status = 400
                        response = {
                            "msg": "Nothing - Not like increment or fav_state change request"
                        }            
            else:
                status = 400
                response = {
...
class AllUserFavs(Handler):
    ...
    @authenticated
    def get(self):
        all_favs = UserFavs.all().filter('user =', self.request.user).order('-fav_time')
        logging.info("all_fav object: %s", UserFavs.all().filter('user =', self.request.user).count()) # PS1: change to logging.debug
        blog_info = []
        if all_favs.count():
            for userfav in all_favs:    
                blog_comment_count = UserComment.all().filter('blog =', userfav.blog).count() or 0
                likes_count = UserLikes.all().filter('blog =', userfav.blog).count() or 0
                blog_info.append((userfav.blog, likes_count, blog_comment_count))
...
class SeeYourPosts(Handler):
    ...
    def get(self):
        ...
        for a in posts:
            blog_id_comment_count[a.key().id()] = UserComment.all().filter('blog =', a).count()
        self.render('see_your_posts.html', posts=posts, fav_blog_id_list=fav_blog_id_list, blog_id_comment_count=blog_id_comment_count)
...
class UserBlogComment(Handler):
    ...
    def get(self, blog_id):
        ...
        blog = Blog.get_by_id(int(blog_id))
        blog_comments = UserComment.all().filter('blog =', blog).order('created_time')
        logging.info("blog_comments = %s, blog_id_type=%s" %(blog_comments, type(blog_id)))
        self.render('comments.html', blog_comments=blog_comments, user=user, blog=blog)
    ...
    def delete(self, blog_id):
        ...
        if blog:
            ...
            if comment and comment.blog.key().id() == blog.key().id():
                ...
                status = 400
                response = {
                    'msg': 'comment doesnot exists or comment blog id and blog id doesnot match'
                }
        else:
            status = 400
            response = {
...
app = webapp2.WSGIApplication(
    [
        ...
        webapp2.Route(r'/blog/<blog_id:\d+>/comments', handler=UserBlogComment),
    ],
    debug=True
)
